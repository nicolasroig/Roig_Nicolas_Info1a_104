...
from APP_FILMS.clients.gestion_clients_wtf_forms import FormWTFAjouterclients
from APP_FILMS.clients.gestion_clients_wtf_forms import FormWTFDeleteclients
from APP_FILMS.clients.gestion_clients_wtf_forms import FormWTFUpdateclients
...
def client_afficher(order_by, Id_client_sel):
        ...
        try:
            ...
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                if order_by == "ASC" and Id_client_sel == 0:
                    strsql_client_afficher = """SELECT Id_client, nom, prenom FROM t_client
                    ...
                    strsql_client_afficher = """SELECT Id_client, nom, prenom FROM t_client 
                    WHERE Id_client = %(value_Id_client_selected)s"""

                    mc_afficher.execute(strsql_client_afficher, valeur_Id_client_selected_dictionnaire)
                else:
                    strsql_client_afficher = """SELECT Id_client, nom, prenom     
                    FROM t_client ORDER BY Id_client DESC"""
                ...
                if not data_client and Id_client_sel == 0:
                    ...
                    flash(f"Le client n'existe pas !!", "warning")
                else:
    ...
    return render_template("clients/client_afficher.html", data=data_client)
